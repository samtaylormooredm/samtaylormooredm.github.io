class ScrollerVis {
  ...
  initVis() {
    ...
    // Identify assignments with the longest time- get top 5 instead of 15
    ...
    // Changed to dataLongestAssignment instead of dataLongestHike
    ...
    // Initialize scales, added colors for easy and difficult
    vis.colorScale = d3.scaleOrdinal()
        .range(['#4682b4', '#9dbdd7', '#ffd966'])
        .domain(['default','inactive', 'highlight']);
  ...
  step1() {
    ...
    vis.rect.transition()
        .attr('fill', d => d.difficulty=='Easy' ? vis.colorScale('highlight') : vis.colorScale('default'));
  }
  ...
  step2() {
    ...
    vis.rect.transition()
        .attr('fill', d => d.difficulty=='Difficult' ? vis.colorScale('highlight') : vis.colorScale('default'));
  }
  ...
  step3() {
    ...
    vis.rect.transition()
        .attr('opacity', 1)
        .attr('fill', d => d.name==vis.dataLongestAssignment.name ? vis.colorScale('highlight') : vis.colorScale('inactive'))
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
        .attr('x', (d, i) => i % vis.config.columns * (vis.config.cellWidth + vis.config.cellSpacing))
        .attr('y', (d, i) => Math.floor(i / vis.config.columns) % vis.config.rows * (vis.config.cellHeight + vis.config.cellSpacing));
